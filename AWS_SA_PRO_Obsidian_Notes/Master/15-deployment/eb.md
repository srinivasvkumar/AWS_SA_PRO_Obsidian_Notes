...
- **Application Version**: specific labeled version of deployable code for an application. The source bundle is stored in [[AWS_SA_PRO_Obsidian_Notes/Master/04-storage/s3|s3]]
...
- Decoupling an existing [[rds]] from an EB environment:
    ...
    7. Locate the `DELETE_FAILED` stack in [[cloudformation]] console, manually delete the stack and pick to [[6r|retain]] stuck resources
...
- We can configure the [[appsync|security]] group as well to allow SSL connections
