...
- **On-Demand (default)**:
    ...
    - Multiple instance types (different sizes) can run on the same [[00_Master_Links_and_Intro|EC2]] hosts, consuming a different allocation of resources
    ...
    - In case of resource shortage the [[Master/Git_hub_notes/AWS-SAP-C02-Notes-main/README|reserved instances]] receive highest priority, consider them instead of on-demand in case of business critical systems
...
- **[[Master/Git_hub_notes/AWS-SAP-C02-Notes-main/README|Spot instances]]**:
    - Cheapest way to get [[00_Master_Links_and_Intro|EC2]] compute capacity
    - Spot pricing is selling [[00_Master_Links_and_Intro|EC2]] capacity at lower price in order make use of spare [[00_Master_Links_and_Intro|EC2]] capacity on the host machines
    - If the spot price goes above selected maximum price, our instances are terminated
    - We should never use the [[Master/Git_hub_notes/AWS-SAP-C02-Notes-main/README|spot instances]] for workloads which can't tolerate interruptions
    - Anything which can tolerate interruptions and can be re-triggered is good for spot
- **Standard [[Master/Git_hub_notes/AWS-SAP-C02-Notes-main/README|Reserved Instances]]**:
    - On-demand is generally used for unknown or short term usage, reserved is for long term consistent usage of [[00_Master_Links_and_Intro|EC2]]
    - Reservations:
        ...
        - Az locked instances reserve [[00_Master_Links_and_Intro|EC2]] capacity
    ...
    - [[Master/Git_hub_notes/AWS-SAP-C02-Notes-main/README|Reserved instances]] are good for components which have known usage, require consistent access for compute for a long term basis
- **Scheduled [[Master/Git_hub_notes/AWS-SAP-C02-Notes-main/README|Reserved Instances]]**:
    - Great for long term requirements which does not run constantly, ex. batch processing running 5 hours/day
    - For scheduled [[Master/Git_hub_notes/AWS-SAP-C02-Notes-main/README|reserved instances]] we specify a time window. The capacity can be used only during the time window
    - [[AWS_SA_PRO_Obsidian_Notes/Master/12-security-and-config/cloudhsm|Limitations]]: 
        - Does not support all kind of instance types
        - Minimum purchase per year is 1200 hours, minimum commitment is 1 year
- **Dedicated Hosts**:
    - They are [[00_Master_Links_and_Intro|EC2]] hosts allocated to a customer entirely
    ...
    - Capacity management:
        ...
        - We have a limited capacity in terms of how many [[00_Master_Links_and_Intro|EC2]] instances we can launch
- **[[Master/Git_hub_notes/AWS-SAP-C02-Notes-main/README|Dedicated Instances]]**:
    - Our instances run on an [[00_Master_Links_and_Intro|EC2]] host with other instances of ours. The host is not shared with other AWS customers, no other customers will use the same hardware
    - We don't pay for the host, nor do we share the host
    - There are some extra fees for this kind of purchase option:
        - One-of hourly fee for any regions in which we are using [[Master/Git_hub_notes/AWS-SAP-C02-Notes-main/README|dedicated instances]]
        - There is a fee for the [[Master/Git_hub_notes/AWS-SAP-C02-Notes-main/README|dedicated instances]] themselves
    - [[Master/Git_hub_notes/AWS-SAP-C02-Notes-main/README|Dedicated instances]] are common in industries where we cannot share hardware
    - No extra capacity management required from us

## Capacity Reservations

- AWS prioritizes any scheduled commitment for delivering [[00_Master_Links_and_Intro|EC2]] capacity
- After scheduled instances on-demand is prioritized
- The leftover capacity can be used for [[Master/Git_hub_notes/AWS-SAP-C02-Notes-main/README|spot instances]]
- Capacity reservation is different compared to [[Master/Git_hub_notes/AWS-SAP-C02-Notes-main/README|reserved instances]]
...
- Saving Plan can be 2 different types:
    ...
    - [[00_Master_Links_and_Intro|EC2]] Saving Plan: up to 72% saving for [[00_Master_Links_and_Intro|EC2]]
    - SageMaker savings plan: reduces costs up to 64% for SageMaker usage. Applies to Sagemaker ML instances (ml.t3, ml.m5, ml.m5d)
- General compute savings plan currently apply to [[00_Master_Links_and_Intro|EC2]], [[Fargate]] and [[lambda]]
...
- [[appsync|Security]] Groups are associated with an ENI, not an [[00_Master_Links_and_Intro|EC2]] instances
- Every instances is allocated a primary private IPv4 address from the subnet range. This IP address remains the within the lifetime of [[00_Master_Links_and_Intro|EC2]] instance
...
- Public IP address is allocated to the instance if we launch it in a subnet where this is enabled or we explicitly enable a primary address to the instance. These public IP addresses are dynamic and they can change if the [[00_Master_Links_and_Intro|EC2]] instance is moving to another [[00_Master_Links_and_Intro|EC2]] host
...
- ENIs can have 1 or more IPv6 addresses, 1 MAC address and 1 or more [[appsync|Security]] Groups
...
- By default source/destination check is enabled, if disabled the ENI can process traffic which was not created by the [[00_Master_Links_and_Intro|EC2]] instances or traffic for which the [[00_Master_Links_and_Intro|EC2]] instance is not the destination

## Bootstrapping and AMI Baking

- Bootstrapping:
    - Is a way of building [[00_Master_Links_and_Intro|EC2]] instances in a flexible way. Isn't fast, but super flexible.
    - Flexible, automated building of [[00_Master_Links_and_Intro|EC2]] instances
    - We provision [[00_Master_Links_and_Intro|EC2]] instances and add a script to the user data
    ...
    - When finished, [[00_Master_Links_and_Intro|ec2]] instance is ready to use.
- AMI Baking:
    ...
    - We launch an [[00_Master_Links_and_Intro|EC2]] instance and perform the necessary tasks from which we can create an AMI
    ...
    - <span style="color: orange;">If there are any exam question related to time taken to launch the [[00_Master_Links_and_Intro|EC2]] instance, think of AMI baking.</span>

## EC2 Placement Groups

- Allow us to influence [[00_Master_Links_and_Intro|EC2]] instance placements, insuring that instances are closed together or not
- There are 3 types of placements groups in AWS:
    ...
    - **Partition**: groups of [[00_Master_Links_and_Intro|EC2]] instances which are spread apart on different host hardware
...
- Ideally the instances in a cluster placement group are located on the same rack, often on the same [[00_Master_Links_and_Intro|EC2]] host
...
- Considerations for cluster placement groups: <span style="color: #ff5733;">Important for EXAM!!!</span>
    ...
    - We can span over [[AWS_SA_PRO_Obsidian_Notes/Master/03-networking/vpc|vpc]] peers, but this will impact performance negatively
...
- Considerations: <span style="color: #ff5733;">Important for EXAM!!!</span>
    ...
    - We can not use [[Master/Git_hub_notes/AWS-SAP-C02-Notes-main/README|dedicated instances]] or hosts
...
- We can launch as many instances as we need in a partition group. We can select the partition by hand or we can let [[00_Master_Links_and_Intro|EC2]] decide on a partition for a new instance
...
- Use cases for [[Master/Git_hub_notes/AWS-SAP-C02-Notes-main/README|spot instances]]: batch jobs or workloads that are resilient to failure
- We can launch [[Master/Git_hub_notes/AWS-SAP-C02-Notes-main/README|spot instances]] with a spot request. A spot request contains the following information:
    - Maximum price
...
- Canceling a [[Master/Git_hub_notes/AWS-SAP-C02-Notes-main/README|spot instances]]: in order ot cancel a spot instance, it has to be in an **open**, **active** or **disabled** state
- Spot instance [[step-functions|states]]:
    ![Spot instance states](spot_request_states.png)
...
- Spot Fleet - set of [[Master/Git_hub_notes/AWS-SAP-C02-Notes-main/README|spot instances]] + (optional) on-demand instances
...
- Strategies to allocate [[Master/Git_hub_notes/AWS-SAP-C02-Notes-main/README|spot instances]]:
...
- Spot fleets allow us to automatically request [[Master/Git_hub_notes/AWS-SAP-C02-Notes-main/README|spot instances]] with the lowest price
...
### Architecture Diagram
![[EC2 Reservations2.png]]


### Architecture Diagram
![[EC2 Reservations1.png]]
