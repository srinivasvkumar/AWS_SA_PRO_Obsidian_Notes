...
- [[step-functions|States]]: `ALARM` or `OK` based on the value of a metric against a threshold over time
...
- CloudWatch Logs is a public service designed to store, monitor and provide access [[vpc-flow-logs|logging]] data
- Can provide [[vpc-flow-logs|logging]] ingestion for AWS products natively, but also for on-premises, [[iot]] or any application
- CloudWatch Agent: used to provide ingestion for custom applications
- CloudWatch can also ingest log streams from [[AWS_SA_PRO_Obsidian_Notes/Master/03-networking/vpc|vpc]] Flow Logs or [[cloudtrail]] (account events and AWS API calls)
...
- Export logs from CloudWatch:
    - **[[AWS_SA_PRO_Obsidian_Notes/Master/04-storage/s3|s3]] Export**: we can create an export task (`Create-Export-Task`) which will take up to 12 hours. Its not real time. It is encrypted using SSE-[[AWS_SA_PRO_Obsidian_Notes/Master/04-storage/s3|s3]].
...
- Subscription filters can be used to create a [[vpc-flow-logs|logging]] aggregation architecture
