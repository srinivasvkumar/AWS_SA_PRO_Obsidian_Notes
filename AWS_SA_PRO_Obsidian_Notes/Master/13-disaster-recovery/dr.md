...
- [[ebs|Instance Store volumes]]:
    ...
    - If the host fails, the [[ebs|instance store volumes]] will also fail
...
- [[ebs]]:
    ...
    - Snapshots of [[ebs]] are stored in [[AWS_SA_PRO_Obsidian_Notes/Master/04-storage/s3|s3]], will increase reliability
...
- DR Architecture - Storage:
    ![DR Architecture - Storage](AWS_SA_PRO_Obsidian_Notes/Master/13-disaster-recovery/images/DRArchitectureStorage.png)
...
- [[lambda]]:
    ...
    - In [[AWS_SA_PRO_Obsidian_Notes/Master/03-networking/vpc|vpc]] mode (private) Lambdas are injected in VPCs. If an AZ fails, [[lambda]] can be automatically injected in another subnet in a different AZ
    - It will take the failure if a region in order for [[lambda]] to be impacted
- DR Architecture - Compute:
    ![DR Architecture - Compute](AWS_SA_PRO_Obsidian_Notes/Master/13-disaster-recovery/images/DRArchitectureCompute.png)
...
- [[rds]]:
    - Requires creation of a subnet group which specifies which subnet can be used in a [[AWS_SA_PRO_Obsidian_Notes/Master/03-networking/vpc|vpc]] for a DB
...
- DR Architecture - Databases:
    ![DR Architecture - Databases](AWS_SA_PRO_Obsidian_Notes/Master/13-disaster-recovery/images/DRArchitectureDatabases.png)

## DR Architecture - Networking

- Networking at local level:
    - [[AWS_SA_PRO_Obsidian_Notes/Master/03-networking/vpc|vpc]] are regionally resilient
    - Certain gateway objects like [[AWS_SA_PRO_Obsidian_Notes/Master/03-networking/vpc|vpc]] Router and IGW are also regionally resilient
...
- Interface endpoint:
    - Are tied to an AZ
    - Multiple [[AWS_SA_PRO_Obsidian_Notes/Master/03-networking/privatelink|interface endpoints]] can be deployed into different AZs
- DR Architecture - Networking:
    ![DR Architecture - Networking](AWS_SA_PRO_Obsidian_Notes/Master/13-disaster-recovery/images/DRArchitectureNetworking.png)
- Global Networking:
    - [[route53]] can route globally to different regions ([[route53|failover routing]])