...
- Standard: 
    ...
    - They don't have performance [[AWS_SA_PRO_Obsidian_Notes/Master/12-security-and-config/cloudhsm|limitations]]
- FIFO: 
...
- It can process large payloads and have the bulk of the payloads stored in [[AWS_SA_PRO_Obsidian_Notes/Master/04-storage/s3|s3]]
- When we send a message using `SendMessage` API, the library uploads the content to [[AWS_SA_PRO_Obsidian_Notes/Master/04-storage/s3|s3]] and stores a link to this content in the message
- When receiving a message, the library loads the payload from [[AWS_SA_PRO_Obsidian_Notes/Master/04-storage/s3|s3]] and replaces the link with the payload in the message
- When deleting a message from a queue, the large [[AWS_SA_PRO_Obsidian_Notes/Master/04-storage/s3|s3]] payload will also be deleted
...
- Every queue (including DLQ) have a [[AWS_SA_PRO_Obsidian_Notes/Master/04-storage/s3|retention period]] for messages. Every message has an `[[mq]]-timestamp` which represents when was the message was added to the queue. In case a message is moved into a DLQ, this timestamp is not altered
- Generally the [[AWS_SA_PRO_Obsidian_Notes/Master/04-storage/s3|retention period]] for a DLQ should be longer compared to normal queues
