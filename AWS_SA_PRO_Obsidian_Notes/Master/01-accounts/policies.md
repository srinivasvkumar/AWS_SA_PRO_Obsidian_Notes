...
- **Resource-based policies**: attach inline policies to resources. The most common examples of resource-based policies are Amazon [[AWS_SA_PRO_Obsidian_Notes/Master/04-storage/s3|s3]] bucket policies and [[iam]] role trust policies. Resource-based policies grant permissions to a principal entity that is specified in the policy. Principals can be in the same account as the resource or in other accounts
...
- Best practice: use least privilege for maximum [[appsync|security]]
...
- Policy Variables and Tags:
    - `${aws:username}`: example `"Resource:["arn:aws:[[Git_hub_notes/certified-aws-solutions-architect-professional-main/04-storage/s3|s3]]:::mybucket/${aws:username}/*"]`
    - AWS Specific:
        - `aws:CurrentTime`
        - `aws:TokenIssueTime`
    ...
    - Service Specific:
        ...
        - `[[Git_hub_notes/certified-aws-solutions-architect-professional-main/04-storage/s3|s3]]:prefix`
        - `[[Git_hub_notes/certified-aws-solutions-architect-professional-main/04-storage/s3|s3]]:max-keys`
        - `[[sns]]:EndPoint`
        - `[[sns]]:Protocol`
    - Tag Based:
...
- Policy evaluation logic - same account:
    ![policy evaluation logic - same account](AWS_SA_PRO_Obsidian_Notes/Master/01-accounts/images/PolicyEvaluation1.png)
- Policy evaluation logic - different account:
    ![policy evaluation logic - different account](AWS_SA_PRO_Obsidian_Notes/Master/01-accounts/images/PolicyEvaluation2.png)
