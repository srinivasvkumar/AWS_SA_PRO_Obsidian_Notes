...
- EMR runs in one AZ only within a [[AWS_SA_PRO_Obsidian_Notes/Master/03-networking/vpc|vpc]] using [[ec2]] for compute
- It can use [[Master/Git_hub_notes/AWS-SAP-C02-Notes-main/README|spot instances]], instance fleets, reserved and on-demand instances as well
- EMR is used for big data processing, manipulation, analytics, indexing, transformation, etc.
- EMR architecture:
    ![EMR architecture](AWS_SA_PRO_Obsidian_Notes/Master/08-data-analytics/images/EMRArchitecture.png)
    ...
    - **Task nodes**: used to only run tasks. If they are terminated, the HDFS storage is not affected. Ideally we use [[Master/Git_hub_notes/AWS-SAP-C02-Notes-main/README|spot instances]] for task nodes
    - EMRFS: file system backed by [[AWS_SA_PRO_Obsidian_Notes/Master/04-storage/s3|s3]], can persist beyond the lifetime of the cluster. Offers lower performance than HDFS, which is based on local volumes
...
- Job run:
    ...
    - When a job is submitted we must specify an [[iam]] role that will provide required access for the job to other services such as [[AWS_SA_PRO_Obsidian_Notes/Master/04-storage/s3|s3]]
