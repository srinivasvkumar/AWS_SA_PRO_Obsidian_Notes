...
- Advanced features of Redshift:
    - RedShift Spectrum: allows querying data from [[AWS_SA_PRO_Obsidian_Notes/Master/04-storage/s3|s3]] without loading it into Redshift platform
...
- Redshift if s [[AWS_SA_PRO_Obsidian_Notes/Master/03-networking/vpc|vpc]] service, it uses [[AWS_SA_PRO_Obsidian_Notes/Master/03-networking/vpc|vpc]] [[appsync|security]]: [[iam]] permissions, [[kms]] encryption at rest, [[cloudwatch]] monitoring
- Redshift Enhance [[AWS_SA_PRO_Obsidian_Notes/Master/03-networking/vpc|vpc]] Routing:
    - By default Redshift uses public routes for traffic when communicating with external services or any public AWS service (such as [[AWS_SA_PRO_Obsidian_Notes/Master/04-storage/s3|s3]])
    - When enabled, traffic is routed based on the [[AWS_SA_PRO_Obsidian_Notes/Master/03-networking/vpc|vpc]] networking configurations (SG, ACLs, etc.)
    - Traffic is routed based on the [[AWS_SA_PRO_Obsidian_Notes/Master/03-networking/vpc|vpc]] networking configuration
    - Traffic can be controlled by [[appsync|security]] groups, it can use network DNS, it can use [[AWS_SA_PRO_Obsidian_Notes/Master/03-networking/vpc|vpc]] gateways
- Redshift architecture:
    ![Redshift architecture](AWS_SA_PRO_Obsidian_Notes/Master/08-data-analytics/images/RedshiftArchitecture.png)
...
-  **Cluster**: a set of nodes, which consists of a leader node and one or more compute nodes
    ...
    - Redshift supports both the [[ec2]]-[[AWS_SA_PRO_Obsidian_Notes/Master/03-networking/vpc|vpc]] and [[ec2]]-Classic [[eb|platforms]] to launch a cluster. We create a cluster subnet group if you are provisioning our cluster in our [[AWS_SA_PRO_Obsidian_Notes/Master/03-networking/vpc|vpc]], which allows us to specify a set of subnets in our [[AWS_SA_PRO_Obsidian_Notes/Master/03-networking/vpc|vpc]]
- **Redshift Nodes**:
...
- Redshift can use [[AWS_SA_PRO_Obsidian_Notes/Master/04-storage/s3|s3]] for backups in the form a snapshots
- There are 2 types of backups:
    ...
    - Manual snapshots: performed after manual triggering, no [[AWS_SA_PRO_Obsidian_Notes/Master/04-storage/s3|retention period]]
...
![Redshift Resilience and Recovery](AWS_SA_PRO_Obsidian_Notes/Master/08-data-analytics/images/RedshiftDR.png)
