...
- [[Master/Git_hub_notes/AWS-SAP-C02-Notes-main/README|RDS]] is often described as a Database-as-a-service (DBaaS) but this is not accurate. It should be named Database Server as a Service (DBSaaS) product
- [[Master/Git_hub_notes/AWS-SAP-C02-Notes-main/README|RDS]] provides managed database instances, which can themselves hold one or more databases
- Benefits of [[Master/Git_hub_notes/AWS-SAP-C02-Notes-main/README|RDS]] are the we don't need to manage the physical hardware, the server operating system or the database system itself
- [[Master/Git_hub_notes/AWS-SAP-C02-Notes-main/README|RDS]] supports MySQL, MariaDB, PostgreSQL, Oracle, Microsoft SQL Server
- Amazon [[aurora]]: it is a db engine created by AWS and we can select it as well for usage
- [[Master/Git_hub_notes/AWS-SAP-C02-Notes-main/README|RDS]] Subnet Group: list of subnets which an [[Master/Git_hub_notes/AWS-SAP-C02-Notes-main/README|RDS]] database can use. Generally it is best practice to have on Subnet Group per database deployment
...
- [[Master/Git_hub_notes/AWS-SAP-C02-Notes-main/README|RDS]] instances come in various types, share many of features of [[ec2]]. Example of instances: db.m5, db.r5, db.t3
- [[Master/Git_hub_notes/AWS-SAP-C02-Notes-main/README|RDS]] instances can be single AZ or multi AZ (active-passive failover)
...
- [[billing]] for [[Master/Git_hub_notes/AWS-SAP-C02-Notes-main/README|RDS]]:
...
- They are 2 types of Multi AZ deployments:
    ...
    - Multi AZ Cluster
- Multi AZ Instance:
    - Used to add resilience to an [[Master/Git_hub_notes/AWS-SAP-C02-Notes-main/README|RDS]] instance
    - Replication happens at the storage level
    - Enables synchronous replication between primary and standby instances
    - Multi AZ is an option which can be enabled on an [[Master/Git_hub_notes/AWS-SAP-C02-Notes-main/README|RDS]] instance, when enabled secondary hardware is allocated in another AZ (standby replica)
    - [[Master/Git_hub_notes/AWS-SAP-C02-Notes-main/README|RDS]] is accessed via provided endpoint address (CNAME)
    ...
    - If an error occurs with the primary instance, [[Master/Git_hub_notes/AWS-SAP-C02-Notes-main/README|RDS]] automatically changes the endpoint to point to the standby replica. This failover occurs in around 60-120 seconds
    ...
    - In case of a failover the DNS name will be updated to point to the standby replica instance. Since this is a DNS change, for the update it generally takes between 60-120 seconds to occur. This can be lessened by removing DNS [[api-gateway|caching]] in the application
    ...
    - Failovers can happen if:
        ...
        - Software patching
- Multi AZ Cluster:
    - [[Master/Git_hub_notes/AWS-SAP-C02-Notes-main/README|RDS]] is capable of having one writer replicate to two reader instances. We can have 2 readers only!
...
- [[Master/Git_hub_notes/AWS-SAP-C02-Notes-main/README|RDS]] backup types:
    - Manual snapshots:
        ...
        - When we delete an [[Master/Git_hub_notes/AWS-SAP-C02-Notes-main/README|RDS]] instance, AWS offers to make one final snapshot
    - Automatic backups:
        ...
        - In addition to the automated snapshots, every 5 minute transaction logs are written to [[AWS_SA_PRO_Obsidian_Notes/Master/04-storage/s3|s3]]
        - Automatic backups are not retained, we can set the [[AWS_SA_PRO_Obsidian_Notes/Master/04-storage/s3|retention period]] between 0 and 35 days
        - Automatic backups can be retained after a DB is deleted, but they still expire after the [[AWS_SA_PRO_Obsidian_Notes/Master/04-storage/s3|retention period]]
...
- Backups are stored in AWS manages [[AWS_SA_PRO_Obsidian_Notes/Master/04-storage/s3|s3]] buckets (backups are not visible to us directly in [[AWS_SA_PRO_Obsidian_Notes/Master/04-storage/s3|s3]]) => any data in [[AWS_SA_PRO_Obsidian_Notes/Master/04-storage/s3|s3]] is regionally resilient
- [[Master/Git_hub_notes/AWS-SAP-C02-Notes-main/README|RDS]] backups are taken from the standby instance in case Multi AZ is enabled
- [[Master/Git_hub_notes/AWS-SAP-C02-Notes-main/README|RDS]] Restores:
    - [[Master/Git_hub_notes/AWS-SAP-C02-Notes-main/README|RDS]] creates a new [[Master/Git_hub_notes/AWS-SAP-C02-Notes-main/README|RDS]] instance when we restore an automated backup or a manual snapshot => new address will be created for the DB
...
- Read replicas are read-only replicas of an [[Master/Git_hub_notes/AWS-SAP-C02-Notes-main/README|RDS]] instance
...
- With all the [[Master/Git_hub_notes/AWS-SAP-C02-Notes-main/README|RDS]] engines we can use encryption in transit (SSL/TLS). This can be set to be mandatory on a per user bases
- For encryption at rest [[Master/Git_hub_notes/AWS-SAP-C02-Notes-main/README|RDS]] supports [[ebs]] volume encryption using [[kms]] which is handled by the host [[ebs]] and it is invisible for the database engine
...
- Oracle supports TDE with [[AWS_SA_PRO_Obsidian_Notes/Master/12-security-and-config/cloudhsm|cloudhsm]], offering much stronger encryption
- [[iam]] [[api-gateway|authentication]] with [[Master/Git_hub_notes/AWS-SAP-C02-Notes-main/README|RDS]]:
    - Normally login is controlled with local database users (username/password)
    - We can configure [[Master/Git_hub_notes/AWS-SAP-C02-Notes-main/README|RDS]] to allow [[iam]] [[api-gateway|authentication]] (only [[api-gateway|authentication]], not authorization, authorization is handled internally!):
    ![ASG Lifecycle Hooks](AWS_SA_PRO_Obsidian_Notes/Master/07-databases/images/RDSIAMAuthentication.png)
...
- [[Master/Git_hub_notes/AWS-SAP-C02-Notes-main/README|RDS]] proxies provide multiplexing: a smaller number of connections can be used to connect to the database while having a larger number of applications using the database through the proxy. This helps to reduce the load on the database
...
- When to use RDS proxy?
    - In case we have [[api-gateway|errors]] such as `Too many connections`. An RDS proxy can reduce the number of connections to the dabase while being able to handle many more connections from the applications to itself
...
- RDS Proxy key facts:
    ...
    - Only accessible from a [[AWS_SA_PRO_Obsidian_Notes/Master/03-networking/vpc|vpc]], not accessible from the public internet
...
- Fills the gap between the main [[Master/Git_hub_notes/AWS-SAP-C02-Notes-main/README|RDS]] product and [[ec2]] running a DB engine
- The main [[Master/Git_hub_notes/AWS-SAP-C02-Notes-main/README|RDS]] is fully managed database service => OS/Engine access is limited
- In contrast databases running on [[ec2]] are self managed, this can have significant management overhead
- RDS custom bridges this gap, we can utilize [[Master/Git_hub_notes/AWS-SAP-C02-Notes-main/README|RDS]] but still get access to customization we would have when running a DB instance on [[ec2]]
- Currently RDS custom works from MSSQL or Oracle
- We can connect to the underlying OS using SSH, RDP or Session Manager
- RDS custom will run withing our AWS account. Classic [[Master/Git_hub_notes/AWS-SAP-C02-Notes-main/README|RDS]] will run in an AWS managed environment
- If we need to perform [[Master/Git_hub_notes/AWS-SAP-C02-Notes-main/README|RDS]] customization for RDS Custom, we need to look inside the Database Automation settings to make sure we wont have any disruption caused by the Database Automation. We need to pause Database Automation for this period
