...
- On-demand backups:
    ...
    - If we restore a backup we can [[6r|retain]] or remove indexes
...
- Available view types:
    ...
    - `NEW_AND_OLD_IMAGE`: stores the before/after [[step-functions|states]] of the items in case of a change
- In some cases the new/old [[step-functions|states]] recorded can be empty, example in case of a deletion the new state of an item is blank
...
- DAX operates within a [[AWS_SA_PRO_Obsidian_Notes/Master/03-networking/vpc|vpc]], designed to be deployed in multiple AZs in a [[AWS_SA_PRO_Obsidian_Notes/Master/03-networking/vpc|vpc]]
...
- When writing data to DynamoDB, DAX uses write-through [[api-gateway|caching]], the data is written at the same time to the cache as it is written to the DB
- DAX is deployed in a VCP! Any application which wants to use DAX has to be in a [[AWS_SA_PRO_Obsidian_Notes/Master/03-networking/vpc|vpc]] as well
