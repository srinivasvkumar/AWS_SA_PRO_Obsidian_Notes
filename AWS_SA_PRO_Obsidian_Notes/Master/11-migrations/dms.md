...
- SCT is a standalone app used for converting one database engine to another including conversion of schema from a DB to [[AWS_SA_PRO_Obsidian_Notes/Master/04-storage/s3|s3]]
...
- Migration steps:
    ...
    2. Ship the device back to AWS. They will load the data into an [[AWS_SA_PRO_Obsidian_Notes/Master/04-storage/s3|s3]] bucket
    3. DMS migrates from [[AWS_SA_PRO_Obsidian_Notes/Master/04-storage/s3|s3]] into a target source
    4. Change Data Capture (CDC) can capture changes and via [[AWS_SA_PRO_Obsidian_Notes/Master/04-storage/s3|s3]] intermediary they are also written to the target database