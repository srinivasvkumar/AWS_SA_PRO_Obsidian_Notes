...
- `Fn::GetAZs` - returns the available AZs in region. If the region has a default [[AWS_SA_PRO_Obsidian_Notes/Master/03-networking/vpc|vpc]] configured, it return the AZs which are available in the default [[AWS_SA_PRO_Obsidian_Notes/Master/03-networking/vpc|vpc]]
...
- Also, it tries to determine a dependency order (example: [[AWS_SA_PRO_Obsidian_Notes/Master/03-networking/vpc|vpc]] => SUBNET => [[ec2]]) by using references or functions
...
- Stacks can have limits:
    ...
    - We can't easily reuse resources, example reference a [[AWS_SA_PRO_Obsidian_Notes/Master/03-networking/vpc|vpc]]
...
- [[appsync|Security]]: we can use self-managed roles or service-managed roles (everything handled by the product). CFN will assume a role to interact with the target accounts
- Terminology:
    ...
    - [[6r|Retain]] Stacks: remove stack instances from a StackSet but [[6r|retain]] the infrastructure
- StackSet use cases:
...
- With deletion policy, we can define on each resource to **Delete** (default), **[[6r|Retain]]** or **Snapshot** (if supported)
- Supported resources for snapshot are: [[ebs]] volumes, [[elasticache]], [[AWS_SA_PRO_Obsidian_Notes/Master/07-databases/neptune|neptune]], [[rds]], [[redshift]]
...
![[AWS_SA_PRO_Obsidian_Notes/Master/EB/14-DeploymentAndManagement/00_LearningAids/CloudFormation-NestedStacks.png]]
