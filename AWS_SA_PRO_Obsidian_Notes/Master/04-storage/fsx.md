...
- Resilient and highly available service. Can be deployed in single or multi-AZ within a [[AWS_SA_PRO_Obsidian_Notes/Master/03-networking/vpc|vpc]]. Even in a single-AZ deployment the backend of the service uses replications to ensure that is resilient to hardware failure
...
- FSx can be accessed over [[AWS_SA_PRO_Obsidian_Notes/Master/03-networking/vpc|vpc]] peering, [[AWS_SA_PRO_Obsidian_Notes/Master/03-networking/vpn|vpn]] and [[DX]]
...
- FSx is available over [[AWS_SA_PRO_Obsidian_Notes/Master/VPN|VPN]] or DX for on-premises
- [[AWS_SA_PRO_Obsidian_Notes/Master/S3|S3]] repository: files are stored in [[AWS_SA_PRO_Obsidian_Notes/Master/S3|S3]] and they are lazily loaded into FSx for Lustre file system at first usage
- Sync changes between the file system and [[AWS_SA_PRO_Obsidian_Notes/Master/S3|S3]]: `hsm_archive` command. The file system and the [[AWS_SA_PRO_Obsidian_Notes/Master/S3|S3]] bucket are not automatically in sync
- Lustre file system:
...
- Baseline performance of the file system is based on the size:
    ...
    - For both types we can burst up to 1300 MB/s per TiB using [[Master/Git_hub_notes/certified-aws-solutions-architect-professional-main/README|credits]]
- FSx for Luster deployment types:
    ...
    - Both of these options provide backups to [[AWS_SA_PRO_Obsidian_Notes/Master/04-storage/s3|s3]] (manual or automatic with a retention of 0-35 days)
...
- Provides reach set of features available with NetApp's data management software:
    ...
    - [[Master/Git_hub_notes/AWS-SAP-C02-Notes-main/README|Disaster recovery]] using SnapMirror and Amazon FSx Backups
    - [[api-gateway|Caching]]: FlexCache, Global File Cache
    - Multiprotocol access from Linux, Windows
    - [[transfer-family|Other features]]: antivirus scanning
...
- Use case for NetApp ONTAP:
    ...
    - FlexCache for [[api-gateway|caching]] data and bringing data closer between regions and on-prem access
