...
![DX architecture](AWS_SA_PRO_Obsidian_Notes/Master/03-networking/images/DirectConnectArchitecture1.png)
...
- The router in the DX location should support Border Gateway Protocol ([[AWS_SA_PRO_Obsidian_Notes/Master/03-networking/bgp|bgp]]) and [[AWS_SA_PRO_Obsidian_Notes/Master/03-networking/bgp|bgp]] MD5 [[api-gateway|Authentication]]
...
- It is a [[appsync|security]] feature that improves/partially improves a long-standing problem with DX: lack of builtin encryption
...
- MACsec key components:
    ...
    - **MACSec Key Agreement** protocol: manages discovery, [[api-gateway|authentication]] and key generation
...
- DX connection process:
![DX Connection Process](DXConnectionProcess.png)
...
- A VIF is simply a [[AWS_SA_PRO_Obsidian_Notes/Master/03-networking/bgp|bgp]] Peering Session => something which exchanges prefixes which allow traffic to be router to one point to the other
- All of this is isolated within a VLAN
- A VLAN isolates different layer 3 networks using VLAN tagging
- [[AWS_SA_PRO_Obsidian_Notes/Master/03-networking/bgp|bgp]] exchanges prefixes, which means each end knows which networks are at each side; BPG MD5 [[api-gateway|authentication]] means that only authenticated data will accepted by either type
...
![DX VIFs - BGP Session + VLAN](DXBGPSessonVLAN.png)
- A single dedicated DX can have in total 50 public/private VIFs, as well as 1 Transit VF
- Hosted VIFs: we can share VIFs with other AWS accounts. Can be connected to a virtual private gateway in a [[AWS_SA_PRO_Obsidian_Notes/Master/03-networking/vpc|vpc]] of the other account

## Private VIFs

- They are used to access the resources inside 1 AWS [[AWS_SA_PRO_Obsidian_Notes/Master/03-networking/vpc|vpc]] using private IP addresses
- Resources can be accessed with their private IP using private VIFs, public IPs and Elastic IPs wont work
- Private VIFs are associated with a Virtual Private Gateway (VGW) which can be associated to 1 [[AWS_SA_PRO_Obsidian_Notes/Master/03-networking/vpc|vpc]]. This has to be in the same region where the DX location connection terminates
- 1 Private VIF = 1 VGW = 1 [[AWS_SA_PRO_Obsidian_Notes/Master/03-networking/vpc|vpc]] (there are ways around this using Transit VIFs)
...
- Creating private VIFs:
    ...
    - We need to enter the [[AWS_SA_PRO_Obsidian_Notes/Master/03-networking/bgp|bgp]] ASN of on-premises (public or private). If private use 64512 to 65535
    - We can choose IPs or auto generate them
    - AWS will advertise the [[AWS_SA_PRO_Obsidian_Notes/Master/03-networking/vpc|vpc]] CIDR range and the [[AWS_SA_PRO_Obsidian_Notes/Master/03-networking/bgp|bgp]] Peer IPs (`/30`)
...
- Private VIFs architecture:
    ![Private VIFs Architecture](DXPrivateVIFS.png)
 - Key learning objectives:
    - Private VIFs are used to access private AWS services
    - Private VIF => 1 VGW => 1 [[AWS_SA_PRO_Obsidian_Notes/Master/03-networking/vpc|vpc]]
    - [[AWS_SA_PRO_Obsidian_Notes/Master/03-networking/vpc|vpc]] needs to be in the same region as the DX location
    - VGW has an AWS assigned
    - Over the private VIF runs the [[AWS_SA_PRO_Obsidian_Notes/Master/03-networking/bgp|bgp]] with IPv4 or IPv6 (separate BPG peering connections)
...
- They offer no direct access to private [[AWS_SA_PRO_Obsidian_Notes/Master/03-networking/vpc|vpc]] services
...
- We can advertise any public IPs we own over [[AWS_SA_PRO_Obsidian_Notes/Master/03-networking/bgp|bgp]], in case we don't have public IPs, we can work with AWS support to allocate some to us
- Public VIFs support bi-directional [[AWS_SA_PRO_Obsidian_Notes/Master/03-networking/bgp|bgp]] communities
...
- Create a public VIF:
    ...
    - Chose the customer side [[AWS_SA_PRO_Obsidian_Notes/Master/03-networking/bgp|bgp]] ASN (ideally this is public ANS for full functionality offered by public VIFs)
    - Configure MD5 [[api-gateway|authentication]] and select optional peering IP addresses
    - We have to select which prefixes we want to advertise
- Public VIFs architecture:
![Public VIFs Architecture](DXPublicVIFS.png)

## Direct Connect Public VIF + VPN

- Using a [[AWS_SA_PRO_Obsidian_Notes/Master/03-networking/vpn|vpn]] gives us an encrypted and authenticated tunnel
- Architecturally, having a [[AWS_SA_PRO_Obsidian_Notes/Master/03-networking/vpn|vpn]] over DX uses a Public VIF + VGW/TGW public endpoints
- With a [[AWS_SA_PRO_Obsidian_Notes/Master/03-networking/vpn|vpn]] we connect to public IPs which belong to a VGW or TGW
- A [[AWS_SA_PRO_Obsidian_Notes/Master/03-networking/vpn|vpn]] is transit agnostic: we can connect using a [[AWS_SA_PRO_Obsidian_Notes/Master/03-networking/vpn|vpn]] to VGW or a TGW over the internet or over DX
- A [[AWS_SA_PRO_Obsidian_Notes/Master/03-networking/vpn|vpn]] is end-to-end encryption between a Customer Gateway (CGW) and TGW/VGW, while MACsec is single hop based
- [[AWS_SA_PRO_Obsidian_Notes/Master/03-networking/vpn|VPNs]] have wide vendor support
- [[AWS_SA_PRO_Obsidian_Notes/Master/03-networking/vpn|VPNs]] have more cryptographic overhead compared to MACsec
- A [[AWS_SA_PRO_Obsidian_Notes/Master/03-networking/vpn|vpn]] can be provided immediately, can be used while DX is provisioned and/or as a DX backup
![VPN over DX](DXPublicVIFVPN.png)
...
- On the AWS side we create VGW associations in any [[AWS_SA_PRO_Obsidian_Notes/Master/03-networking/vpc|vpc]] in any AWS regions
...
![DX Gateway Architecture](AWS_SA_PRO_Obsidian_Notes/Master/03-networking/images/DirectConnectGateway3.png)
...
- DX-TGW Architecture:
   ![DX-TGW Architecture](DXGateway4.png)
...
![DX resilience NONE](AWS_SA_PRO_Obsidian_Notes/Master/03-networking/images/DirectConnectResilience1.png)
- Resilient DX architecture:
![DX resilience OK](AWS_SA_PRO_Obsidian_Notes/Master/03-networking/images/DirectConnectResilience2.png)
- Improved resilient DX architecture:
![DX resilience BETTER](AWS_SA_PRO_Obsidian_Notes/Master/03-networking/images/DirectConnectResilience3.png)
- Extreme resilient DX architecture:
![DX resilience GREAT](AWS_SA_PRO_Obsidian_Notes/Master/03-networking/images/DirectConnectResilience4.png)
...
![DX LAG](AWS_SA_PRO_Obsidian_Notes/Master/03-networking/images/DirectConnectLAG.png)
